...
export const filterBySearch = (partners, searchQuery) => {
  ...
  return partners.filter((partner) => {
    ...
    const partnerType =
      partner.partner_institution?.typeOfOrganization || partner.type || "";
    const contactName =
      partner.partner_contact_person?.name || partner.contact || "";
...
export const applyFilters = (partners, filters) => {
  return partners.filter((partner) => {
    ...
    const partnerType =
      partner.partner_institution?.typeOfOrganization || partner.type || "";
    const partnerStatus = partner.status || "";
    const partnerDuration =
      partner.duration_of_partnership || partner.duration || "";
...
export const sortPartners = (partners, column, direction) => {
  ...
  sortedPartners.sort((a, b) => {
    ...
    if (column === "name") {
      ...
      valueA = getDurationInMonths(
        a.duration_of_partnership || a.duration || ""
      );
      valueB = getDurationInMonths(
        b.duration_of_partnership || b.duration || ""
      );
    } else if (column === "contact") {
      valueA = a.partner_contact_person?.name || a.contact || "";
      valueB = b.partner_contact_person?.name || b.contact || "";
...
export const getUniqueValues = (partners, field) => {
  ...
  if (field === "type") {
    ...
    values = partners.map(
      (partner) =>
        partner.aau_contact?.interestedCollegeOrDepartment || partner.college
    );
  } else if (field === "contact") {
    values = partners.map(
      (partner) => partner.partner_contact_person?.name || partner.contact
    );
  } else {
